...
function RoCUI_ToggleMinimapButton()
    if RoCUIDB_Options["Minimap_Button"] == true then
...
SlashCmdList.RoCUIMINIMAP = function()
    if RoCUIDB_Options["Minimap_Button"] == true then
		Settings.SetValue("RoCUI_Options_Minimap_Button", false, true)
	else
		Settings.SetValue("RoCUI_Options_Minimap_Button", true, true)
	end
	RoCUI_ToggleMinimapButton()
end